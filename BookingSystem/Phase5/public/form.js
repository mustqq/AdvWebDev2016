...
const lines = errors.map((e) => {
  let field = e.field || "field";
  
  // 1) Fix the label at the start
  if (field === "resourceName") field = "Resource Name";
  if (field === "resourceDescription") field = "Description";

  // 2) Fix the actual message text from the server
  let msg = e.msg || "Invalid value";
  msg = msg.replace("resourceName", "Resource Name"); // Removes the technical name from the middle
  msg = msg.replace("resourceDescription", "Description");

  return `• ${field}: ${msg}`;
});

  return `We couldn't create the resource due to some invalid fields. Please fix the following:\n\n${lines.join("\n")}`;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        const msg = "A resource with this name already exists in the system. Please choose a unique name or check the current list.";
        showFormMessage("info", `Conflict: Duplicate Name\n\n${msg}`); // Better title
        return;
      }
    ...
    const msgLines = [];
    msgLines.push(`✅ Resource "${body?.data?.name ?? ""}" added successfully!`);
    msgLines.push(`Created at: ${createdAt}`);
    msgLines.push(`Database Reference ID: ${body?.data?.id ?? ""}`);
